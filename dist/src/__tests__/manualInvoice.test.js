...
describe("POST /products/invoice/manual", () => {
    ...
    it("deducts stock for carton items and records purchases", async () => {
        ...
        await (0, productController_1.processInvoiceManual)(req, res);
        if (res.statusCode !== 200) {
            // Debug: print captured error calls
            // eslint-disable-next-line no-console
            console.log("manualInvoice error calls:", consoleSpy.mock?.calls);
            // eslint-disable-next-line no-console
    ...
    it("validates required fields and returns 400 on missing customerName/items", async () => {
        const makeRes = () => ({ statusCode: 200, body: null, status(n) { this.statusCode = n; return this; }, json(obj) { this.body = obj; } });
        const r1 = makeRes();
        await (0, productController_1.processInvoiceManual)({ body: { customerName: "", items: [] } }, r1);
        expect(r1.statusCode).toBe(400);
        const r2 = makeRes();
        await (0, productController_1.processInvoiceManual)({ body: { customerName: "Bob" } }, r2);
        expect(r2.statusCode).toBe(400);
        const r3 = makeRes();
        await (0, productController_1.processInvoiceManual)({ body: { items: [] } }, r3);
