...
const downloadBackup = async (req, res) => {
    try {
        ...
        const [users, products, sales, purchases, expenses, customers, customerPurchases, invoices, invoiceItems, payments, customerGroups, salesAgents, locations, stores, branches, featureFlags, userPermissions, invoiceMeta, pcsInventory, supportMessages, expenseCategories, banks, expenseBanks, supplierPurchaseMeta, supplierPayments, suppliers,] = await Promise.all([
            prisma_1.default.users.findMany({ where: { tenantId } }),
            prisma_1.default.products.findMany({ where: { tenantId } }),
            prisma_1.default.sales.findMany({ where: { tenantId } }),
            prisma_1.default.purchases.findMany({ where: { tenantId } }),
            prisma_1.default.expenses.findMany({ where: { tenantId } }),
            prisma_1.default.customers.findMany({ where: { tenantId } }),
            prisma_1.default.customerPurchases.findMany({ where: { tenantId } }),
            prisma_1.default.invoices.findMany({ where: { tenantId } }),
            prisma_1.default.invoiceItems.findMany({ where: { tenantId } }),
            prisma_1.default.payments.findMany({ where: { tenantId } }),
            prisma_1.default.customerGroups.findMany({ where: { tenantId } }),
            prisma_1.default.salesAgents.findMany({ where: { tenantId } }),
            prisma_1.default.locations.findMany({ where: { tenantId } }),
            prisma_1.default.stores.findMany({ where: { tenantId } }),
            prisma_1.default.branches.findMany({ where: { tenantId } }),
            prisma_1.default.featureFlags.findMany({ where: { tenantId } }),
            prisma_1.default.userPermissions.findMany({ where: { tenantId } }),
            prisma_1.default.invoiceMeta.findMany({ where: { tenantId } }),
            prisma_1.default.pcsInventory.findMany({ where: { tenantId } }),
            prisma_1.default.supportMessages.findMany({ where: { tenantId } }),
            prisma_1.default.expenseCategories.findMany({ where: { tenantId } }),
            prisma_1.default.banks.findMany({ where: { tenantId } }),
            prisma_1.default.expenseBanks.findMany({ where: { tenantId } }),
            prisma_1.default.supplierPurchaseMeta.findMany({ where: { tenantId } }),
            prisma_1.default.supplierPayments.findMany({ where: { tenantId } }),
            prisma_1.default.suppliers.findMany({ where: { tenantId } }),
        ]);
        const backupData = {
            meta: {
    ...
    try {
        ...
        if (Array.isArray(data.users)) {
            for (const item of data.users) {
                await prisma_1.default.users.upsert({
        ...
        if (Array.isArray(data.products)) {
            for (const item of data.products) {
                await prisma_1.default.products.upsert({
        ...
        if (Array.isArray(data.salesAgents)) {
            for (const item of data.salesAgents) {
                await prisma_1.default.salesAgents.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.locations)) {
            for (const item of data.locations) {
                await prisma_1.default.locations.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.stores)) {
            for (const item of data.stores) {
                await prisma_1.default.stores.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.userPermissions)) {
            for (const item of data.userPermissions) {
                await prisma_1.default.userPermissions.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.supportMessages)) {
            for (const item of data.supportMessages) {
                await prisma_1.default.supportMessages.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.expenseCategories)) {
            for (const item of data.expenseCategories) {
                await prisma_1.default.expenseCategories.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.banks)) {
            for (const item of data.banks) {
                await prisma_1.default.banks.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.expenseBanks)) {
            for (const item of data.expenseBanks) {
                await prisma_1.default.expenseBanks.upsert({
                    where: { id: item.id },
                    update: item,
                    create: item,
                });
        ...
        if (Array.isArray(data.customerGroups)) {
            for (const item of data.customerGroups) {
                await prisma_1.default.customerGroups.upsert({
        ...
        if (Array.isArray(data.branches)) {
            for (const item of data.branches) {
                try {
                    await prisma_1.default.branches.upsert({
                        where: { id: item.id },
                        update: item,
                        create: item,
                    });
                    ...
                    console.warn(`Skipping branch ${item.id}: parent store missing?`);
                }
            }
        }
        ...
        if (Array.isArray(data.sales)) {
            for (const item of data.sales) {
                try {
                    await prisma_1.default.sales.upsert({
                    ...
                    console.warn(`Skipping sale ${item.saleId}: parent product missing?`);
                }
            }
        }
        ...
        if (Array.isArray(data.purchases)) {
            for (const item of data.purchases) {
                try {
                    await prisma_1.default.purchases.upsert({
                    ...
                    console.warn(`Skipping purchase ${item.purchaseId}: parent product missing?`);
                }
            }
        }
        ...
        if (Array.isArray(data.expenses)) {
            for (const item of data.expenses) {
                try {
                    await prisma_1.default.expenses.upsert({
                    ...
                    console.warn(`Skipping expense ${item.expenseId}`);
                }
            }
        }
        ...
        if (Array.isArray(data.customerPurchases)) {
            for (const item of data.customerPurchases) {
                try {
                    await prisma_1.default.customerPurchases.upsert({
                        where: { id: item.id },
                        update: item,
                        create: item,
                    });
                    ...
                    console.warn(`Skipping customer purchase ${item.id}`);
                }
            }
        }
        ...
        if (Array.isArray(data.invoices)) {
            for (const item of data.invoices) {
                try {
                    await prisma_1.default.invoices.upsert({
                    ...
                    console.warn(`Skipping invoice ${item.invoiceId}: parent missing?`);
                }
            }
        }
        ...
        if (Array.isArray(data.invoiceItems)) {
            for (const item of data.invoiceItems) {
                try {
                    await prisma_1.default.invoiceItems.upsert({
                        where: { id: item.id },
                        update: item,
                        create: item,
                    });
                    ...
                    console.warn(`Skipping invoice item ${item.id}: parent invoice missing?`);
                }
            }
        }
        ...
        if (Array.isArray(data.payments)) {
            for (const item of data.payments) {
                try {
                    await prisma_1.default.payments.upsert({
                        where: { id: item.id },
                        update: item,
                        create: item,
                    });
                    ...
                    console.warn(`Skipping payment ${item.id}: parent invoice missing?`);
                }
            }
        }
        ...
        if (Array.isArray(data.supplierPurchaseMeta)) {
            for (const item of data.supplierPurchaseMeta) {
                try {
                    ...
                    console.warn(`Skipping supplier meta ${item.id}`);
                }
            }
        }
        ...
        if (Array.isArray(data.supplierPayments)) {
            for (const item of data.supplierPayments) {
                try {
                    await prisma_1.default.supplierPayments.upsert({
                        where: { id: item.id },
                        update: item,
                        create: item,
                    });
                    ...
                    console.warn(`Skipping supplier payment ${item.id}`);
                }
            }
        }
        ...
        if (Array.isArray(data.invoiceMeta)) {
            for (const item of data.invoiceMeta) {
                try {
                    ...
                    console.warn(`Skipping invoice meta ${item.id}`);
                }
            }
        }
