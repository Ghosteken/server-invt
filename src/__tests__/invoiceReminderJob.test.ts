...
jest.mock("../services/notificationService", () => ({
  __esModule: true,
  appendNotification: (...args: any[]) => appendNotificationMock(...args),
}));

const getInvoiceMetaMock = jest.fn();
jest.mock("../services/invoiceMetaService", () => ({
  __esModule: true,
  getInvoiceMeta: (...args: any[]) => getInvoiceMetaMock(...args),
}));

import { runInvoiceReminderScan } from "../jobs/invoiceReminderJob";
...
describe("runInvoiceReminderScan", () => {
  ...
  it("notifies 3 days before due date and marks dueSoonNotifiedAt", async () => {
    ...
    await runInvoiceReminderScan(new Date("2026-01-14T15:00:00.000Z"));
