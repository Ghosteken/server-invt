import { cacheGet, cacheSet, withCache } from '../services/cache';
...
describe('cache service', () => {
  ...
  test('set/get stores values and respects TTL in memory fallback', async () => {
    await cacheSet('k1', { a: 1 }, 1); // TTL 1s
    const v1 = await cacheGet('k1');
  ...
  test('withCache loads and serializes value and reuses until TTL', async () => {
    ...
    const first = await withCache('k2', 5, loader);
    expect(first).toEqual({ n: 42 });
    expect(loaderCalls).toBe(1);
    const second = await withCache('k2', 5, loader);
    expect(second).toEqual({ n: 42 });
    expect(loaderCalls).toBe(1);
  });

  test('Redis path gracefully falls back when REDIS_URL is absent', async () => {
    const v = await withCache('k3', 1, async () => 'x');
    expect(v).toBe('x');
    const read = await cacheGet('k3');
    expect(read).toBe('x');
  });
});