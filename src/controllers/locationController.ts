...
export const createLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try { appendNotification({ type: "location", message: `Location created: ${created.name}`, actorUserId: req.user?.userId, tenantId }); } catch {}
...
export const updateLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try { appendNotification({ type: "location", message: `Location updated: ${existing.name} → ${updated.name}`, actorUserId: req.user?.userId, tenantId }); } catch {}
...
export const deleteLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try { appendNotification({ type: "location", message: `Location deleted: ${existing.name}`, actorUserId: req.user?.userId, tenantId }); } catch {}
