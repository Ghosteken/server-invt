...
export const createLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existing = await prisma.locations.findFirst({ where: { tenantId, name } });
    if (existing) { res.status(409).json({ message: "Location already exists" }); return; }
    const created = await prisma.locations.create({ data: { id: randomUUID(), name, tenantId } });
...
export const updateLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!name) { res.status(400).json({ message: "Location name is required" }); return; }
    const tenantId = (req as any).tenantId || req.user?.tenantId || "default";
    const existing = await prisma.locations.findFirst({ where: { id, tenantId } });
